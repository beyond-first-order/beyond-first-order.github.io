...
<details>
  <summary> <b>Amir Gholami</b>: <i>Applied Second Order Methods for Diagnostics, Training, and Fast Inference</i> </summary>
  
Second order methods provide us with powerful tools that we can use for various tasks in analyzing, training, and even efficient deployment of NNs. In this talk, I will focus on practical applications of second order methods for problems relevant in production today. First, I will start by showing use cases where Hessian can be used as a diagnostics tool which enables us to systematically analyze loss landscape. In particular, I will discuss results for analyzing large batch training as well as studying the correlation between architectural changes and Hessian spectrum. Second, we will discuss how second order information could provide efficient ways in better adapting learning rate during training than first order methods. In particular, we will discuss AdaHessian which uses Hessian diagonal to adjust the learning rate. Finally, I will show our recent results on using Hessian as a systematic method to quantize and prune NNs, achieving SOTA results.
</details>
